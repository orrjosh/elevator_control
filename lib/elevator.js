...
const UP = 1;
const DOWN = 0;
const INVALID_INPUT_MESSAGE = `Invalid input, selected floors must all be between ${BOTTOM_FLOOR} and ${TOP_FLOOR}`;
...
module.exports.move = (
  ...
  if (!isSelectedFloorsValid(selectedFloors)) {
    // would include a better error message
  ...
  while (selectedFloors.length !== 0) {
    ...
    if (direction === UP && Math.max(...selectedFloors) > floor) {
      ...
      // this would probably be more readable with an object
      floor = ar[0];
      actionsLog = ar[1];
